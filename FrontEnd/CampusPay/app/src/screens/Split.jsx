...
export default function SplitPaymentScreen() {
  const { prevamount } = useLocalSearchParams();
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      try {
        ...
        const newResults = results.filter(
          (user) => !currentlySelectedIds.has(user._id)
        );
  ...
  const handleSplit = async () => {
    if (!amount || !description.trim() || selectedUsers.length === 0) {
      Alert.alert(
        "Missing Information",
        "Please enter an amount, description, and select at least one person."
      );
      return;
    }
      ...
      >
        ...
        <View style={styles.footer}>
          <TouchableOpacity
            style={[
              styles.splitBtn,
              (!amount || selectedUsers.length === 0 || isSubmitting) &&
                styles.splitBtnDisabled,
            ]}
            onPress={handleSplit}
